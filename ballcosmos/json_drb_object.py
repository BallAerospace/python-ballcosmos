...
import http.client
import json
from .json_rpc import JsonRpcRequest, \
                      JsonRpcResponse, \
...
class JsonDRbObject:
    """Class to perform JSON-RPC Calls to the COSMOS Server
        (or other JsonDrb server)
    ...
    def disconnect(self):
        ...
        # Cannot set @connection to None here because this method
        #   can be called by
        # other threads and @connection being None would cause
        #   unexpected errors in method_missing
        # Also don't want to take the mutex so that we can interrupt
        #   method_missing if necessary
    ...
    def write(self, method_name, *method_params):
        ...
        return -- The result of the method call. If the method raises
                  an exception the same exception is also raised.
                  If something goes wrong with the protocol
                  a DRb::DRbConnError exception is raised.
        """

        with self.mutex:
            # This flag and loop are used to automatically reconnect and retry
            #   if something goes
            # wrong on the first attempt writing to the socket.
            #   Sockets can become disconnected
            # between function calls, but as long as the remote server
            #   is back up and running the
            # call should succeed even when it discovers a broken socket
            ...
            while True:
                ...
                response = self.make_request(method_name,
                                             method_params,
                                             first_try)
                if response is None:
                    self.disconnect()
    ...
    def connect(self):
        ...
        try:
            self.connection = http.client.HTTPConnection(self.hostname,
                                                         self.port)
            ...
            try:
                ...
                self.disconnect()
                self.connection = None
                raise RuntimeError("Connect error: {}".format(err))
        except Exception as e:
    ...
    def handle_response(self, response_data):
        # The code below will always either raise
        #   or return breaking out of the loop
        if response_data is not None:
            ...
            if isinstance(response, JsonRpcErrorResponse):
                ...
                # else:
                raise RuntimeError("JsonDRb Error ({:d}): {:s}"

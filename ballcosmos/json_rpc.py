...
class JsonRpcRequest(JsonRpc):
    ...
    def __init__(self, method_name, method_params, id):
        ...
        method_params -- Array of strings which represent
    ...
    def params(self):
        """Returns the array of strings which represent
        the parameters to send to the method
        """
        try:
    ...
    def from_json(cls, request_data):
        """Creates and returns a JsonRpcRequest object from
        a JSON encoded String.

        The version must be 2.0 and the JSON must include
        the method and id members.
        ...
        try:
            ...
            # Verify the jsonrpc version is correct
            #   and there is a method and id
            if not (hash['jsonrpc'] == "2.0"
                    and 'method' in hash
                    and 'id' in hash):
                raise RuntimeError()
...
class JsonRpcResponse(JsonRpc):
    ...
    def from_json(cls, response_data):
        ...
        The version must be 2.0 and the JSON must include the id members.
        It must also include either result for success
        or error for failure but never both.
...
class JsonRpcError(JsonRpc):
    ...
    def from_hash(cls, hash):
        ...
        Parameters:
        hash -- Hash containing the following keys: code,
                message, and optionally data
        """
        if 'code' in hash \
                and (int(hash['code']) == hash['code']) \
                and 'message' in hash:
